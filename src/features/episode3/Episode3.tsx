...
export function Episode3({ ep, userId }: { ep: Episode; userId?: string }) {
  ...
  function progressFor(): { pct: number; label: string } {
    ...
    const labels: Record<Phase, string> = {
      dlg: 'EP.3 · Our Schedule', phrase: 'Career · Rookie', manners: 'Career · Rookie',
  ...
  function next() {
    if (phase === 'dlg') {
      ...
      else { setPhase('phrase'); if (userId) void setSkill(userId, 'Time & Date', 'mastered'); addEvent('ep03-dialogue'); }
      return;
    }
    if (phase === 'phrase') { setPhase('manners'); addEvent('ep03-phrases'); return; }
    if (phase === 'manners') {
      if (mannersIdx + 1 < manners.length) { setMannersIdx(mannersIdx + 1); setMannersOk(false); }
      else { setPhase('write'); addEvent('ep03-manners'); }
      return;
    }
    if (phase === 'write') {
      ...
      else { setPhase('reward'); addEvent('ep03-write'); }
      return;
    }
  }
  ...
  function finishNaming() {
    ...
    if (userId) {
      ...
      void unlock(userId, 'photocard-003', 'photocard');
      void setCareer(userId, 'rookie', 40);
      void setEpisodeDone(userId, ep.no);
    } else {
      applyGuestProgress({ stageName: stageName.trim(), careerStage: 'rookie', careerPct: 40, episode: ep.no, unlockId: 'photocard-003' });
    }
      ...
      <div className="ep__body">
        <div className="scene" key={phase} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-4)' }}>
          ...
          {phase === 'manners' && manners[mannersIdx] && (
            <CombineGame question={manners[mannersIdx]} onAnswered={(c) => { setMannersOk(c); addEvent('ep03-manners-' + mannersIdx, c); }} />
          )}
          {phase === 'write' && wtask && (
            <WritingPractice task={wtask} charIdx={charIdx} onCharDone={(isLast) => { setWriteDone(true); if (isLast) addEvent('ep03-write-word'); }} />
          )}
          {phase === 'reward' && (
            <>
              {!named && (
